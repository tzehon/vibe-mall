...
import { TREND_MALL_FAVICON_HREF } from "@/lib/storefrontFavicon";
...
function campaignNameFromTrend(trend: string) {
  const words = trend
    .trim()
    .split(/\s+/)
    .filter(Boolean)
  ...
  return words.length ? `${words.join(" ")} Shelf` : "Trend Mall Shelf";
}

function buildMicroPitch(trend: string, product: ProductSearchResult, index: number) {
...
function generateDemoHtml({ trend, products }: GenerateStorefrontInput) {
  const campaignName = campaignNameFromTrend(trend);
    ...
    .map((product, index) => {
      ...
      return `<article class="product-card" style="--card-accent: ${borderColor}">
        ...
        <div class="card-copy">
          ...
          <h2>${safeName}</h2>
          <p class="pitch">${buildMicroPitch(trend, product, index)}</p>
...
<html lang="en">
  <head>
    ...
    <link rel="icon" href="${TREND_MALL_FAVICON_HREF}" type="image/svg+xml" />
  ...
  <body>
    <main>
      <section class="hero">
        <div>
          <p class="kicker">Trend Mall campaign</p>
          <h1>${escapeHtml(campaignName)}</h1>
          <p class="why">This mall shelf exists because ${safeTrend} deserves a shoppable drop that feels curated, cinematic, and ready to share.</p>
        </div>
...
export async function generateStorefrontHtmlWithCodex({
  ...
  if (process.env.CODEX_DEMO_MODE === "true") {
    const html = generateDemoHtml({ trend, products });
  ...
  const prompt = buildCodexStorefrontPrompt({ trend, products });
