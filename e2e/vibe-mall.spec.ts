...
const demoEmail = "demo@vibemall.local";
const demoPassword = "vibe-mall-demo";
const secondDemoEmail = "second@vibemall.local";
const secondDemoPassword = "vibe-mall-second";
const e2eVibe = "Playwright cute birthday shelf";
...
async function cleanupStorefront(storefrontId: string | null) {
  ...
  try {
    ...
    await client
      ...
      .deleteOne({ _id: new ObjectId(storefrontId), vibe: e2eVibe });
  } finally {
    await client.close();
  }
}
...
test.describe("seeded demo storefront flow", () => {
  test.skip(
    ...
    "Requires MONGODB_URI, MONGODB_DB, AUTH_SECRET, and a seeded demo account."
  );
  ...
  test("generates a draft, renders it in a sandboxed iframe, and publishes a public URL", async ({
    ...
    try {
      ...
      await page.getByRole("textbox", { name: "Vibe" }).fill(e2eVibe);
      ...
      await expect(
        page
          ...
          .getByText("Generated by Codex for Vibe Mall")
      ).toBeVisible();
      ...
      await expect(embedPage.getByText("Generated by Codex for Vibe Mall")).toBeVisible();
